...
| `007-芯片ID读取`                                | 读取芯片唯一ID、Flash容量、芯片型号，通过RS232串口打印，并伴随LED闪烁                           |
...
#### 9. `007-芯片ID读取`
- **主要功能**：读取 STM32H750 的芯片唯一 ID（96位）、Flash 容量（单位 KB）和芯片类型，通过 RS232 串口（UART8）周期性输出信息，同时 LED 闪烁指示运行状态。
- **关键代码**：
  - `chip_info_get_cpu_id()` 读取 CPU ID（三个 32 位寄存器）
  - `chip_info_get_flash_size()` 读取 Flash 大小
  - `chip_info_get_chip_type()` 读取芯片类型（预留）
- **适用场景**：芯片信息验证、生产测试、软件版本识别等需要读取硬件唯一标识的应用。
- **使用说明**：连接 RS232 串口（波特率 115200），上电后每 500ms 输出一次信息，同时 LED（PA0）闪烁。

> **注意**：工程中已包含 MPU 缓存配置、SysTick 延时、串口驱动等基础模块，可直接作为芯片信息读取的最小系统模板。
...
- **需要读取芯片硬件信息（UID/Flash Size）**：使用 `007-芯片ID读取` 工程。
