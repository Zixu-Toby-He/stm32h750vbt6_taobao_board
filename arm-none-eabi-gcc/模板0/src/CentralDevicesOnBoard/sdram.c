...
void SDRAM_Init(void)
{
	...
	SDRAM_Handler.Init.SDBank             = FMC_SDRAM_BANK1;                       // SDRAM 接在 BANK5 上
...
void SDRAM_Initialization_Sequence(SDRAM_HandleTypeDef *hsdram)
{
	...
	// SDRAM 控制器初始化完成后，还需要按照如下顺序初始化 SDRAM
	SDRAM_Send_Cmd(0, FMC_SDRAM_CMD_CLK_ENABLE, 1, 0);          // 时钟配置使能
	delay_us(500);                                              // 至少延时 200us
	// wait_us(500);                                              // 至少延时 200us
	...
	temp = (u32)
		SDRAM_MODEREG_BURST_LENGTH_1          |	         // 设置突发长度：1（取值范围：1、2、4、8）
		SDRAM_MODEREG_BURST_TYPE_SEQUENTIAL   |	         // 设置突发类型：连续（取值范围：连续、交错）
		SDRAM_MODEREG_CAS_LATENCY_2           |	         // 设置  CAS 值：3（取值范围：2、3）
		SDRAM_MODEREG_OPERATING_MODE_STANDARD |          // 设置操作模式：0（标准模式）
		SDRAM_MODEREG_WRITEBURST_MODE_SINGLE;            // 设置突发写模式：1（单点访问）
	SDRAM_Send_Cmd(0, FMC_SDRAM_CMD_LOAD_MODE, 1, temp);    // 设置 SDRAM 的模式寄存器

	// 刷新频率计数器（以SDCLK频率计数）,计算方法:
	// COUNT = SDRAM刷新周期 / 行数 - 20 = SDRAM刷新周期(us) * SDCLK频率(MHz) / 行数
	// 根据使用手册：
	//     SDRAM刷新周期 = 64 ms
	//     SDCLK         = 200 MHz / 2 = 100 MHz
	//     行数          =  8192（2^13）
	// 故：COUNT = 64*1000*100/8192-20=677
...
void HAL_SDRAM_MspInit(SDRAM_HandleTypeDef *hsdram)
{
	...
	// 初始化 PD0、PD1、PD8 ~ PD10、PD14、PD15
...
u8 SDRAM_Send_Cmd(u8 bankx,u8 cmd,u8 refresh,u16 regval)
{
	...
	// 向 SDRAM 发送命令
	// return (HAL_SDRAM_SendCommand(&SDRAM_Handler, &Command, 0XFFFF) != HAL_OK);
...
// 在指定地址（Bank5_SDRAM_ADDR + WriteAddr）开始，连续写入 n 个字节
// pBuffer：字节指针
// WriteAddr：要写入的地址
// n：要写入的字节数
...
// 在指定地址（Bank5_SDRAM_ADDR + WriteAddr）开始，连续读出 n 个字节.
// pBuffer：字节指针
// ReadAddr：要读出的起始地址
// n：要写入的字节数
