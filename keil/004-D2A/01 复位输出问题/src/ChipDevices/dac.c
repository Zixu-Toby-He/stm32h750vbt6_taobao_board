...
DAC_ChannelConfTypeDef DAC_Config;   //DAC1通道1
...
{
	chn = (dac1_chn_t)(channel & 3);

	DAC_Handler.Instance = DAC1;                // 只有一个 DAC 外设
	HAL_DAC_Init(&DAC_Handler);               	// 初始化 DAC

	DAC_Config.DAC_SampleAndHold           = DAC_SAMPLEANDHOLD_DISABLE;
	DAC_Config.DAC_Trigger                 = DAC_TRIGGER_NONE;                   // 不使用触发功能
	DAC_Config.DAC_OutputBuffer            = DAC_OUTPUTBUFFER_DISABLE;           // DAC 输出缓冲关闭
	...
	if (channel & 1)
	{
		HAL_DAC_ConfigChannel(&DAC_Handler, &DAC_Config, DAC_CHANNEL_1); // 配置 DAC 通道 1
		HAL_DAC_Start(&DAC_Handler, DAC_CHANNEL_1);                         // 开启 DAC 通道 1
	}
	if (channel & 2)
	{
		HAL_DAC_ConfigChannel(&DAC_Handler, &DAC_Config, DAC_CHANNEL_2); // 配置 DAC 通道 2
		HAL_DAC_Start(&DAC_Handler, DAC_CHANNEL_2);                         // 开启 DAC 通道 2
	}
