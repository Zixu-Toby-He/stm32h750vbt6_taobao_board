...
static uint32_t get_tim3_clock(void)
{
...
void TIM3_Init(float time_ms, uint16_t ncounts, void (*time_out_handler)(void))
{
	...
	// 总周期数
	total_period = (uint32_t) (time_ms * get_tim3_clock() * 1e-3);
	...
	info_tim_uni.tim_select     = tim_select_tim3_g;
...
// 设置新的定时周期（动态调整）
void TIM3_SetPeriod(uint32_t new_period)
{
	// 先停止定时器
	TIM3_Stop();

	// 更新周期值
	htim.Init.Period = new_period;
