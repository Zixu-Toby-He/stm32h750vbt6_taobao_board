#include "dac.h"


DAC_HandleTypeDef      DAC_Handler;     //DAC句柄
DAC_ChannelConfTypeDef DACCH1_Config;   //DAC1通道1

dac1_chn_t chn;

// 初始化DAC
void DAC_Init(dac1_chn_t channel)
{
	chn = (dac1_chn_t)(channel & 3);
	DAC_Handler.Instance = DAC1;
	HAL_DAC_Init(&DAC_Handler);               	//初始化DAC

	DACCH1_Config.DAC_Trigger      = DAC_TRIGGER_NONE;                   // 不使用触发功能
	DACCH1_Config.DAC_OutputBuffer = DAC_OUTPUTBUFFER_DISABLE;           // DAC 输出缓冲关闭

	if (channel & 1)
	{
		HAL_DAC_ConfigChannel(&DAC_Handler, &DACCH1_Config, DAC_CHANNEL_1); // 配置 DAC 通道 1
		HAL_DAC_Start(&DAC_Handler, DAC_CHANNEL_1);                         // 开启 DAC 通道 1
	}
	if (channel & 2)
	{
		HAL_DAC_ConfigChannel(&DAC_Handler, &DACCH1_Config, DAC_CHANNEL_2); // 配置 DAC 通道 2
		HAL_DAC_Start(&DAC_Handler, DAC_CHANNEL_2);                         // 开启 DAC 通道 2
	}

}

// DAC底层驱动，时钟配置，引脚 配置
// 此函数会被HAL_DAC_Init()调用
// hdac:DAC句柄
void HAL_DAC_MspInit(DAC_HandleTypeDef* hdac)
{      
    GPIO_InitTypeDef GPIO_Initure;
	
    __HAL_RCC_DAC12_CLK_ENABLE();           //使能DAC时钟
    __HAL_RCC_GPIOA_CLK_ENABLE();			//开启GPIOA时钟

    GPIO_Initure.Pin  = 0;
	if (chn & 1)
	{
		GPIO_Initure.Pin |= GPIO_PIN_4;    //PA4
	}
	if (chn & 2)
	{
		GPIO_Initure.Pin |= GPIO_PIN_5;    //PA5
	}
    GPIO_Initure.Mode = GPIO_MODE_ANALOG;           //模拟
    GPIO_Initure.Pull = GPIO_NOPULL;                //不带上下拉
    HAL_GPIO_Init(GPIOA, &GPIO_Initure);
}

//设置通道1输出电压
//mV：0 ~ 2500，代表 0 ~ 2.5V
void DAC_Set_MilliVolt_CH1(u16 mV)
{
	double temp = mV;
	temp /= 1000;
	temp = temp * 4096 / 3.3;
	
	HAL_DAC_SetValue(&DAC_Handler, DAC_CHANNEL_1, DAC_ALIGN_12B_R, temp); // 12位右对齐数据格式设置DAC值
}
//设置通道1输出电压
//mV：0 ~ 3300，代表0~3.3V
void DAC_Set_MilliVolt_CH2(u16 mV)
{
	double temp = mV;
	temp /= 1000;
	temp = temp * 4096 / 3.3;
	
	HAL_DAC_SetValue(&DAC_Handler, DAC_CHANNEL_2, DAC_ALIGN_12B_R, temp); // 12位右对齐数据格式设置DAC值
}
