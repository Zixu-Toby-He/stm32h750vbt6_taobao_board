...
int main(void)
{
	...
	eeprom_page_data_t page_data_ano;
	...
	while(true)
	{
		...
		if (is_data_in())
		{
			...
			for(; (i < temp) && (msg_in[i] != '\r') && (msg_in[i] != '\n'); i++)
			{
		...
		for(i = 0; i < 256; i += 8)
		{
			...
			page_data_ano = eeprom_read_page(i / 8);
			sprintf(
				msg_out,
				"page_%i: [ 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x ]\r\n",
				i / 8,
				page_data.data[0],
				page_data.data[1],
				page_data.data[2],
